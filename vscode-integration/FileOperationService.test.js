...
`;
    ...
    suite('createFile', () => {
        test('新しいファイルを作成する', () => {
            const result = fileOperationService.createFile(testDir, 'new.txt', 'content', '新しいファイル', ['テスト']);
...
`;
    ...
    suite('キャラクターメタデータ操作', () => {
        ...
        test('存在しないファイルにキャラクター設定を追加しようとするとエラーを返す', () => {
            const result = fileOperationService.setCharacterImportance(testDir, 'nonexistent.txt', 'main');
            assert.strictEqual(result.success, false);
            assert.ok(result.message.includes('見つかりません'));
        });
    });
});
//# sourceMappingURL=FileOperationService.test.js.map