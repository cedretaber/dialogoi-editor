...
#### Phase 3.2: その他UI改善 🔄 **進行中**

##### Phase 3.2a: タグ編集機能の改善 ✅ **完了**
- [x] 「タグを追加」ボタンをインラインテキスト入力フィールドに置き換え
- [x] 各タグに削除ボタン（×）を追加
- [x] Enterキーでタグ追加（プロンプトダイアログ不要）
- [x] 重複タグの自動検証
- [x] MetaYamlServiceにタグ操作メソッド追加（addFileTag、removeFileTag、updateFileTags）
- [x] 包括的なテストケース作成（10個の新規テスト）

**実装成果**:
- ✅ より直感的なタグ編集UI
- ✅ リアルタイムでの追加・削除操作
- ✅ TreeViewとWebViewの同期更新

##### Phase 3.2b: その他UI改善項目 **未着手**
- [ ] ドラッグ&ドロップでの並び替え改善
- [ ] 関連ファイルの追加削除の改善（ドラッグ&ドロップでの追加など）
- [ ] 検索・フィルタリング機能
  - タグでフィルター、関連ファイルでフィルター、など
- [ ] ファイルの新規作成・名前変更機能の改善（インライン編集など）
- [ ] 「小説を開始する」ボタンの表示
  - 現在開いているルートディレクトリに dialogoi.yaml が存在しない場合、小説プロジェクトを新規作成する
- [ ] ハイパーリンク機能
- [ ] コンテキストメニューの充実
- [ ] キーボードショートカットの実装
- [ ] 多言語対応（日本語優先）
