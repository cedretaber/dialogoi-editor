  ...
  - invalid: yaml: content`;
    ...
    suite('saveMetaYamlAsync', () => {
      ...
      test('バリデーションエラーがある場合falseを返す', async () => {
        ...
        // .dialogoi-meta.yamlファイルが作成されていないことを確認
        const metaUri = mockFileRepository.createFileUri(`${testDir}/.dialogoi-meta.yaml`);
        const exists = await mockFileRepository.existsAsync(metaUri);
...
files:
  - name: ${fileName}
    ...
    suite('非同期版タグ操作テスト', () => {
      test('updateFileTagsAsyncでタグを正常に更新する', async () => {
        const testDir = '/test/project';
        const fileName = 'chapter1.txt';
        const meta: MetaYaml = {
          readme: 'README.md',
        ...
        // タグを更新
        const newTags = ['新タグ1', '新タグ2', '新タグ3'];
        const result = await service.updateFileTagsAsync(testDir, fileName, newTags);
        assert.strictEqual(result, true);
      ...
      test('addFileTagAsyncで新しいタグを追加する', async () => {
        const testDir = '/test/project';
        const fileName = 'chapter1.txt';
        const meta: MetaYaml = {
          readme: 'README.md',
        ...
        // 新しいタグを追加
        const result = await service.addFileTagAsync(testDir, fileName, '新タグ');
        assert.strictEqual(result, true);
      ...
      test('removeFileTagAsyncでタグを削除する', async () => {
        const testDir = '/test/project';
        const fileName = 'chapter1.txt';
        const meta: MetaYaml = {
          readme: 'README.md',
        ...
        // タグを削除
        const result = await service.removeFileTagAsync(testDir, fileName, 'タグ2');
        assert.strictEqual(result, true);
