...
export class FileOperationService {
  ...
  /**
   * ディレクトリを異なる階層に移動する（非同期）
   */
  async moveDirectoryAsync(
    ...
    try {
      ...
      // 移動元がディレクトリかどうかチェック
      const sourceStats = await this.fileRepository.statAsync(sourceDirUri);
  ...
  /**
   * ディレクトリ移動時の内部ファイルのリンク更新（非同期）
   */
  private async updateLinksForDirectoryMoveAsync(
    oldDirPath: string,
    newDirPath: string,
  ): Promise<void> {
    if (!this.linkUpdateService || !this.pathNormalizationService) {
    ...
    try {
      ...
      await this.walkDirectoryForLinkUpdateAsync(oldDirPath, newDirPath, oldDirPath, newDirPath);
    } catch (error) {
      console.warn(
  ...
  /**
   * ディレクトリを再帰的に走査してファイルのリンクを更新（非同期）
   */
  private async walkDirectoryForLinkUpdateAsync(
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (stat.isDirectory()) {
            // ディレクトリの場合は再帰的に処理
            await this.walkDirectoryForLinkUpdateAsync(
              oldEntryPath,
              newEntryPath,
              rootOldDirPath,
              rootNewDirPath,
