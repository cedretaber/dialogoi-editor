...
- [x] コメント・TODO機能（追加・編集・削除、ステータス管理、マークダウン対応）
...
### 🎯 Phase 3.5b: 残りのUI改善 ✅ **主要機能完了**
- [x] 伏線設定の複数「張る」位置対応（スキーマ拡張）✅ **完了**
  - [x] データ構造の変更（単一対1から複数対1へ）
  - [x] ForeshadowingServiceのCRUD操作実装
...
- [x] ファイル新規作成・名前変更のインライン編集 ✅ **2025-01-22完了**
  - [x] ファイル詳細パネル内でのファイル名インライン編集機能
  - [x] クリック→編集→自動保存のワークフロー実装
  - [x] リアルタイムバリデーション（150msデバウンス）
  - [x] キーボードショートカット（Enter/Escape）対応
  - [x] VSCode標準エクスプローラー準拠の挙動（エディタ状態保持）
  - [x] 非同期ファイル操作によるパフォーマンス向上
  - [x] エラーハンドリングとユーザーフィードバック
- [x] コメント・TODO統合UI ✅ **2025-01-25完了**
  - [x] 専用コメントパネル（サイドバー独立配置）
  - [x] マークダウンレンダリング・TODO機能統合
  - [x] インライン編集とプレビュー機能
  - [x] GitHub風行番号形式（#L42, #L4-L7）
  - [x] posted_by自動取得（dialogoi.yamlから）
- [ ] 管理対象外ファイルの可視化改善
- [ ] コンテキストメニューの充実
- [ ] キーボードショートカットの実装
- [ ] 多言語対応（日本語優先）
...
- webview の component を app 毎に階層に分ける
  - 共通コンポーネントやその他共通コードは別途括り出す
- マークダウンの成形にライブラリを導入
  - remark が良さそう？
  - Markdown 周りの機能も切り出して依存を局所化・テストを容易に
...
- [ ] ドラッグ&ドロップ機能（TreeViewからWebViewへ）
- [ ] 伏線の状態アイコン表示と進捗バー
- [ ] 専用編集画面（WebViewPanelによる広画面UI）
...
1. **コメント・TODO管理システム** ✅ **実装完了**
   - [x] 執筆メモ・TODOの一元管理
   - [x] 行番号による位置追跡とハッシュによる変更検知
   - [x] マークダウン・TODOチェックボックス機能
   - [x] ステータス管理（open/resolved）
