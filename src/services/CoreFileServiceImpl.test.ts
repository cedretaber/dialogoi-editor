...
describe('CoreFileService テストスイート', () => {
  ...
  beforeEach(() => {
    ...
    (
      ...
      return Promise.resolve(content); // テストでは常に文字列を返す
    });
    ...
    mockFileRepository.rmAsync.mockImplementation((uri: Uri, options?: { recursive?: boolean }) => {
      if (!directories.has(uri.path)) {
        return Promise.reject(new Error(`Directory not found: ${uri.path}`));
      }
    ...
    // renameAsyncのモック
    mockFileRepository.renameAsync.mockImplementation((oldUri: Uri, newUri: Uri) => {
      ...
      if (content !== undefined) {
        ...
        return Promise.reject(new Error(`Path not found: ${oldUri.path}`));
      }
      return Promise.resolve();
    });
...
`,
    ...
    mockLinkUpdateService.updateLinksAfterFileOperation.mockImplementation(
    ...
    // テスト用に追加したメソッド
    (
      mockLinkUpdateService as MockProxy<ProjectLinkUpdateService> & {
        clearUpdateCalls: () => number;
        getUpdateCalls: () => Array<{ oldPath: string; newPath: string }>;
      }
    ).clearUpdateCalls = (): number => (updateCalls.length = 0);
    (
      mockLinkUpdateService as MockProxy<ProjectLinkUpdateService> & {
        clearUpdateCalls: () => number;
        getUpdateCalls: () => Array<{ oldPath: string; newPath: string }>;
      }
    ).getUpdateCalls = (): Array<{ oldPath: string; newPath: string }> => updateCalls;
  });
